...
TODO:
    ...
    ***Tasks containg four em-dashes ("*----") represent being accomplished.
    ...
    *(If users want it added) make it so CLIp functions don't throw errors (non-critical errors anyways) due to the necessity of extra try...catch blocks. (Yeah, that would be nice. I'd rather wait till users try it though.)
    ...
    */
    ...
    CLIp.DefineMainObject = function(mainObject){
        ...
        if(!checkPropertyNameLegality(mainObject)){
            if(modes.strictMode){
                throw new NameError("(CLIp Initializer): Main object name \"" + mainObject + "\" has an illegal character. Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
            else{
                console.warn("(CLIp Initializer): Main object name \"" + mainObject + "\"has an illegal character (this will message with user accessibility). Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
        }
    ...
    CLIp.BindPropertyContainer = function(object,objectName){
        ...
        if(!checkPropertyNameLegality(objectName)){
            if(modes.strictMode){
                throw new NameError("(CLIp Initializer): Property container name \"" + objectName + "\" has an illegal character. Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
            else{
                console.warn("(CLIp Initializer): Property container name \"" + objectName + "\"has an illegal character (this will message with user accessibility). Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
        }
    ...
    CLIp.BindProperty = function(mainObject,propertyArray,listenersAllowed){
        ...
        if(!checkPropertyNameLegality(propertyArray[0])){
            if(modes.strictMode){
                throw new NameError("(CLIp Initializer): Property name \"" + propertyArray[0] + "\" has an illegal character. Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
            else{
        ...
        mainObject.__defineSetter__("setValue",function(value){
            if(this.allowListeners && this.listeners.length < 1){
                this.value = value;
            }
            else{
                this.listeners.forEach(function(v){
                    /*Calls the listener and passes in the new value as the first argument, the old value as the second argument and the property type as the third.*/
                    /*If it's a method then it fires the event listener with just the value that it will call the function with an array containing the arguments.*/
                    v(value,this.value,this.type);
                });
                this.value = value;
            }
        });
    ...
    CLIp.BindMethod = function(mainObject,methodArray,listenersAllowed){
        ...
        if(!checkPropertyNameLegality(methodArray[0])){
            if(modes.strictMode){
                throw new NameError("(CLIp Initializer): Method name \"" + methodArray[0] + "\" has an illegal character. Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
            else{
                console.warn("(CLIp Initializer): Method name \"" + methodArray[0] + "\"has an illegal character (this will message with user accessibility). Legal characters are lower-case and upper-case alphabet letters and numbers.");
            }
        }
        if(listenersAllowed){
    ...
    CLIp.BindListener = function(object,callback){
        ...
        if(object.type === "property-container" || object.type === "main-object") throw new InitializationError("(CLIp Initializer): Cannot add a listener to an object with a type \"property-container\" or \"main-object\" (this might be added in a later version).");
        if(object.type !== "property" && object.type !== "method") throw new InitializationError("(CLIp Initializer): Cannot add a listener to an object without a valid or non-existing type.");
        if(object.allowListeners !== true && modes.strictMode === true) throw new InitializationError("(CLIp Initializer): Cannot add a listener to an object not explicitly configured to do so in strict mode.");
        /*In strict mode that will save a lot of time and memory. Imagine waiting for a property to change that never will (lol).*/
        if(object.propertyAccessibility === "read-only" && modes.strictMode) throw new InitializationError("(CLIp Initializer): Cannot add a listener to an object with a property accessibility of \"read-only\" in strict mode.");
        ...
        object.listeners.forEach(function(v){
            if(v === callback){
                if(modes.strictMode === true){
                    throw new InitializationError("(CLIp Initializer): Cannot declare a listener on the object in strict mode because another listener of the same function exists.");
                }
                hasCallback = true;
            }
        });
    ...
    CLIp.RemoveListener = function(object,callback){
        ...
        if(typeof Object.prototype.toString.call(object) !== typeof Object.prototype.toString.call({})) throw new TypeError("Parameter \"object\" (argument 1) is not a valid object.");
        if(typeof callback !== typeof function(){}) throw new TypeError("Parameter \"callback\" (argument 2) is not a valid function.");
        if(object.type === "property-container" || object.type === "main-object") throw new InitializationError("(CLIp Initializer): Cannot remove a listener to an object with a type \"property-container\" or \"main-object\" (this might be added in a later version).");
        if(object.type !== "property" && object.type !== "method") throw new InitializationError("(CLIp Initializer): Cannot remove a listener to an object without a valid or non-existing type.");
        if(object.allowListeners !== true && modes.strictMode === true) throw new InitializationError("(CLIp Initializer): Cannot remove a listener to an object not explicitly configured to do so in strict mode.");
        if(object.propertyAccessibility === "read-only" && modes.strictMode) throw new InitializationError("(CLIp Initializer): Cannot remove a listener to an object with a property accessibility of \"read-only\" in strict mode.");
        ...
        let parseStatementSetter = function(cmd,prop){
            return new Promise(function(res,rej){
                ...
                if(prop.type === "main-object" || prop.type === "property-container"){
                    if(startCollecting) rej(new ParsetimeError("(CLIp parser): Cannot set or execute a main object/property container."));
                }
                if(typeof prop.type === typeof void(0) && startCollecting){
                    rej(new ParsetimeError("(CLIp parser): Property's type cannot be determined (\"property\",\"method\", \"property-container\" or \"main-object\") meaning that it is not changable (Main objects and property-containers are not changable)."));
                    return;
                }
                if(prop.type === "method"){
                    isProperty = false;
